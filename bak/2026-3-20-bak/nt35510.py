...
class NT35510:
    def __init__(self, wr=18, cs=19, dc=20, rst=21, bl=22, rd=26, width=400, height=800):
    ...
    @micropython.viper
    def _cmd_slow(self, c: int):
        #self.dc.low()
        #self.cs.low()
        self._bus_write16_cmd(c)
        #self.cs.high()

    @micropython.viper
    def _data_slow(self, c: int):
        #self.dc.high()
        #self.cs.low()
        self._bus_write16_data(c)
        #self.cs.high()
    ...
    @micropython.viper
    def set_window_slow(self, x0: int, y0: int, x1: int, y1: int):
        _cmd: object = self._cmd
        _data: object = self._data
    ...
    def fb_text(self,text,x=0,y=0,color=-1,bg=0):
        str_length = len(text)*10
        fb = self.MyFrameBuffer(str_length,10)
        if bg:
            fb.fill(bg)
