...
class NT35510:
    ...
    def _claim_bus_pio(self):
        self._ensure_sm()

    def _claim_bus_gpio(self):
        for i in range(16):
            Pin(i, Pin.OUT, value=0)
        self.wr = Pin(self.wr_pin, Pin.OUT, value=1)
        self.cs = Pin(self.cs_pin, Pin.OUT, value=self.cs.value())
        self.dc = Pin(self.dc_pin, Pin.OUT, value=self.dc.value())
        self.rst = Pin(self.rst_pin, Pin.OUT, value=self.rst.value())
        self.rd = Pin(self.rd_pin, Pin.OUT, value=self.rd.value())
