...
class Font:
    ...
    @micropython.viper
    def _fast_blit(self, fb:object, v:int, x0:int, palette: object):
        fb_p = ptr16(fb.buffer)
        fb_width: int = int(fb.width)
        pal_p = ptr16(palette.buffer)
        start:int = v * int(self.bytes_per_char)
        char_bits = ptr8(self.data)
        
        #gfb = self._glyph_fb[v]
        y: int = 0
        x: int = 0
        w: int = int(self.char_width)
        h: int = int(self.char_height)
        bytes_per_row: int = (w+3) // 4
        while y < h:
            x: int = 0
            while x < w:
                gindex = start + y * bytes_per_row + x // 4
                pix = (char_bits[gindex] >> (x % 4 * 2)) & 3
                #fpix = int(gfb.pixel(x, y))
                c = int(pal_p[pix])
                fb_p[y * fb_width + x + x0] = c
                #fb.pixel(x0 + x, y, c)
                x += 1
