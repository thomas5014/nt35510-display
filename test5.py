...
class Font:
    ...
    def wrap_text(self, text, fb, ix, iy, width, force_cut=False, lim=(0,480), key=0, palette=None):
        fb = fb(width,self.char_height)
        fb.fill(color565(*BG))
        if not text:
            return ix, self.char_height
        y = iy
        x = ix
        line = ""
        next = False
        ...
        end_y = 0
        if force_cut:
            wrdwidth = self.get_width(text)
            if wrdwidth > width:
                length = 0
                new_wrd = ""
                for ch in text:
                    ch_l = self.get_width(ch)
                    length += ch_l
                    if length > width:
                        new_wrd += " "
                        length = ch_l
        ...
        try:
            for word in text.split():
                if word == "/n":
                    ...
                    word += " "
                wrdwidth = self.get_width(word)
                if x + wrdwidth < width and not next:
                    line += word
                    x += wrdwidth
                else:
                    ...
                    y += self.char_height
                    next = False
                    line = word if word != "/n" else ""
            if y > lim[0]-self.char_height and y < lim[1]: 
                end_y = self.draw_text(line,fb,ix,y,key,palette)
                end_in_bounds = True
            nt.draw_framebuf(ix, iy, fb)
    ...
    def wrap_text_viper(self, text, FBClass, ix, iy, width, force_cut=False, lim=(0,480), key=0, palette=None):
        if not text:
    ...
    def wrap_text_viper_in(self, b: object, FBClass: object,
                        ...
                        force_cut: int, lim0: int, lim1: int,
...
# font1 = Font("/InterItalic-16-17x20.raw2")
...
example = "Hello World! aeyshdvfudvtbyktvdtfbyhiuguvtjbnkhgrvfbguylbony;o9"
example1 = "hi"
...
if test == 0:
    ...
    for i in range(len(items)):
        ...
        example = f"{items[i][:-5]} {example[:40]}"
        # font1.draw_text2(example, MyFrameBuffer, 20, 0+i*18,0,None)
        font1.draw_text_bytes(example, MyFrameBuffer, 2, 0+i*20,0,None)
elif test == 2:
    example = example_long
    font1.wrap_text_fast(example,MyFrameBuffer, 0, 0, 480)
elif test == 3:
    example = example_long
    font1.wrap_text_viper(example,MyFrameBuffer,0,0,480)
t1 = time.ticks_us()
t2 = time.ticks_us()
t3 = time.ticks_cpu()
# font1.get_width(example)
t4 = time.ticks_cpu()
print(f"draw_text: prep fb {t1-t0:,}us, draw chars, control {t2-t1:,}us - {t4-t3:,}cpu, letters per second: {len(example)*times / ((t1-t0) / 1_000_000):,.2f}, us per letter: {(t1-t0) / (len(example)*times):,}")
